...
class WIB_CFGS(LLC, FE_ASIC_REG_MAPPING):
    ...
    def femb_safe_powering(self, fembs = [], bias_ilim=0.5, dc0_ilim=0.9,dc1_ilim=0.9, dc2_ilim=1.9):
        ...
        if len(fembs) > 0:
            ...
            for femb_id in fembs:
                ...
                if True:
                    ...
                    time.sleep(a)
                    self.femb_power_en_ctrl(femb_id=femb_id, vfe_en=1, vcd_en=0, vadc_en=0, bias_en=bias_en )
                    time.sleep(a)
                    self.femb_power_en_ctrl(femb_id=femb_id, vfe_en=0, vcd_en=0, vadc_en=1, bias_en=bias_en )
                    time.sleep(a)
                    self.femb_power_en_ctrl(femb_id=femb_id, vfe_en=0, vcd_en=1, vadc_en=0, bias_en=bias_en )
                    time.sleep(a)
                    self.femb_power_en_ctrl(femb_id=femb_id, vfe_en=1, vcd_en=1, vadc_en=0, bias_en=bias_en )
                    time.sleep(a)
                    self.femb_power_en_ctrl(femb_id=femb_id, vfe_en=1, vcd_en=1, vadc_en=1, bias_en=bias_en )
                    t1 = time.time()
                    time.sleep(a)
    ...
    def femb_LN2QC_powering(self, fembs=[]):
        if len(fembs) > 0:
            ...
            for femb_id in fembs:
                vols = self.vol_cal(vms_dict, femb_id=femb_id)
                cdldo1 = (vols['CDVDDA'] >= 3) and (vols['CDVDDA'] <= 1.2)
                cdldo2 = (vols['CDVDDIO_HALF'] >= 3) and (vols['CDVDDIO_HALF'] <= 2.35)
                adclldo1 = (vols['ADCLVDDD1P2'] >= 3) and (vols['ADCLVDDD1P2'] <= 1.2)
                adclldo2 = (vols['ADCLP25V_HALF'] >= 3) and (vols['ADCLP25V_HALF'] <= 2.35)
                adcrldo1 = (vols['ADCRVDDD1P2'] >= 3) and (vols['ADCRVDDD1P2'] <= 1.2)
                adcrldo2 = (vols['ADCRP25V_HALF'] >= 3) and (vols['ADCRP25V_HALF'] <= 2.35)
                felldo = (vols['FELVDDP'] >= 3) and (vols['FELVDDP'] <= 1.95)
                ferldo = (vols['FERVDDP'] >= 1.75) and (vols['FERVDDP'] <= 1.95)
                if not (cdldo1 & cdldo2 & adclldo1 & adclldo2 & adcrldo1 & adcrldo2 & felldo & ferldo):
                    # self.por[femb_id] = False
                    print('warning')
...
# input(3)
    ...
    def wib_fe_mon(self, femb_ids=[0, 1, 2, 3], adac_pls_en=0, rst_fe=0, mon_type=2, mon_chip=0, mon_chipchn=0, snc=0,
        ...
        adcss = []
        # input('debug')
        a = self.wib_mon_adcs()  # get rid of previous result
        for i in range(sps):
    ...
    def wib_fe_dac_mon(self, femb_ids, mon_chip=0, sgp=False, sg0=0, sg1=0, vdacs=range(64), sps=3):
        self.wib_mon_switches(dac0_sel=0, dac1_sel=0, dac2_sel=0, dac3_sel=0, mon_vs_pulse_sel=0, inj_cal_pulse=0)
        # for femb_id in femb_ids:
        #     self.femb_cd_gpio(femb_id=femb_id, cd1_0x26=0x00, cd1_0x27=0x1f, cd2_0x26=0x00, cd2_0x27=0x1f)
        self.set_fe_reset()
