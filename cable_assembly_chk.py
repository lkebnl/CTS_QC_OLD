...
import components.cable_assembly_report as a_repo
...
if save:
    ...
    fembNo={}
    for i in fembs:
        fembName['cable{}'.format(i)]=input("Cable{}ID:".format(i)).strip()
        fembNo['femb{}'.format(i)]=fembName['cable{}'.format(i)][1:]
    logs['Cable ID'] = fembName
    # logs['femb id']=fembNo
    logs['date']=datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S")

    datadir=a_func.Create_data_folders_cable(fembName, env, "0pF")
...
############################################
#   create report dir
datareport = a_func.Create_report_folders_cable(fembs, fembName, env, "0pF", datadir)
...
#     adcrst = chk.wib_fe_dac_mon(femb_ids=fembs, mon_chip=mon_chip, sgp=True, vdacs=vdacs, sps=5 )
##################################
...
a_func.mon_path_ana(fembs, mon_refs, mon_temps, mon_adcs, datareport, fembNo, env, NewWIB, ground = 20)
