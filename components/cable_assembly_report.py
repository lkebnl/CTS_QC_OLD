...
def final_report(datareport, fembs, fembNo, Rail=True):
    ...
    for ifemb in fembs:
        ...
        if Rail:
            dict_list = [
                log.report_log021, log.report_log03,
                log.report_log07,
                 log.report_log111
            ]
        else:
            dict_list = [
                log.report_log021, log.report_log03,
                log.report_log07, log.report_log111
            ]

        issue_note = ""
        if all_true[femb_id]:
            # 若所有测试通过，则标记为绿色通过状态
            summary = "<span style='color: green;'>" + "Cable # {}\t      PASS\t    ".format(fembNo['femb%d' % ifemb]) + "</span>"
            note = "### See the Report"
            status = 'P'
        else:
            # 若存在测试失败，则标记为红色失败状态，并输出问题详情
            print(femb_id)
            summary = "<span style='color: red;'>" + "Cable id {}\t      faild\t the assembly checkout".format(fembNo['femb%d' % ifemb]) + "</span>"
        ...
        fhtml = datareport[ifemb] + 'report_Cable_H{}_Slot{}_{}.html'.format(fembNo['femb%d' % ifemb], ifemb, status)
        with open(fhtml, 'w', encoding="utf-8") as file:
            ...
            # if Rail:
            #     if (log.report_log04[femb_id]["Result"] == True):
            ...
            # else:
            # if (log.report_log04[femb_id]["Result"] == True):
            #     Head03 = '<h3><span style="color: green;">PART 02 Digital Interface &nbsp;&nbsp; &lt; Pass &gt;</span></h3>\n'
            #     temp = 'Clock, Fast Command, Data Links'
            # else:
            #     Head03 = '<h3><span style="color: red;">PART 03 SE Interface Measurement &nbsp;&nbsp; | Fail</span></h3>\n'
            # #
            # file.write(Head03)
            ...
            if log.report_log111[femb_id]["Result"] == True:
                ...
                temp = f'<h2>All 128 Channels show normal response, which means digital communication, including clock, fast command, 4*1.25Gbps data links is good</h2>'
            ...
            # 表格：监测路径测量数据
            info = dict_to_html_table(log.report_log12[femb_id], KEY="Monitor Path", VALUE="MonPath")
            file.write(info + '<br>\n')
