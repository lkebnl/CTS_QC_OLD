...
def Create_data_folders_cable(fembNo, env, toytpc):
    datadir = "./CHK/"
    for key,femb_no in fembNo.items():
        datadir = datadir + "Cable_{}".format(femb_no)
    if not os.path.exists(datadir):
        try:
            os.makedirs(datadir)
        except OSError:
...
def Create_report_folders_cable(fembs, fembNo, env, toytpc, datadir):
    ...
    for ifemb in fembs:
        femb_no = fembNo['cable%d'%ifemb]
        plotdir = reportdir + "Cable{}_{}_{}".format(femb_no, env, toytpc)
...
def power_ana(fembs, ifemb, femb_id, pwr_meas, env):
    ...
    check_issue = []
    # parameter
    bias_v_ref = 5; bias_v_err = 2;    bias_i_ref = 0; bias_i_err = 0.5
    ...
    for ifemb in fembs:
        ...
        for i in nchips:  # 8 chips per board
            ...
            if env == 'LN': # long cable effect the vssa ref
                ...
                fe_bgp_ref = 1150; fe_bgp_err = 500
                vcmi_ref = 890; vcmi_err = 500
                vcmo_ref = 1185; vcmo_err = 500
                vrefp_ref = 1930; vrefp_err = 500
                vrefn_ref = 440; vrefn_err = 200
            else:
                vssa_ref = 150; vssa_err = 150
                fe_t_ref = 900; fe_t_err = 300
                fe_bgp_ref = 1150; fe_bgp_err = 200
                vcmi_ref = 865; vcmi_err = 200
                vcmo_ref = 1160; vcmo_err = 200
                vrefp_ref = 1900; vrefp_err = 200
                vrefn_ref = 440; vrefn_err = 200

            #   vssa
            vssa_tmp = abs(round(mon_adcs[f'chip{i}']["VSSA"][1][0][ifemb] * fadc - ground, 1))
        ...
        log.report_log12[femb_id]["ASIC #"] = ["0", "1", "2", "3", "4", "5", "6", "7"]
        log.report_log12[femb_id]["FE T"] = ["{}".format(v) for v in fe_t]
        log.report_log12[femb_id]["FE BGP"] = ["{}".format(v) for v in fe_bgp]
