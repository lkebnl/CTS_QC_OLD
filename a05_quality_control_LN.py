...
def QC_Process(QC_TST_EN=None, input_info=None, item = 0):
    while True:
        QCresult = cts.cts_ssh_FEMB(root=root_path, QC_TST_EN=QC_TST_EN, input_info=input_info, item = item)
        if QCresult != None:
            QCstatus = QCresult[0]
            badchips = QCresult[1]
            break
...
def FEMB_QC(input_info, item = 0):
    ...
    QC_Process(QC_TST_EN=3, input_info=input_info, item = item)  # QC
    return 0
...
print(Fore.GREEN + "A_LN00 : Install FEMB boards, check the connection of Data and Power Cables" + Style.RESET_ALL)
...
inform = cts.read_csv_to_dict(csv_file, 'LN')
print('\n')
FEMB_QC(input_info=inform, item = 100)
