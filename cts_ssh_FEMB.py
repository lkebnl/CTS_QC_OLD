...
# ==============================
    ...
    if QC_TST_EN == 1:
        ...
        if 'LN' in tmp:
            ...
            command1 = ["ssh", "root@192.168.121.123", "cd BNL_CE_WIB_SW_QC;  python3 top_femb_powering.py {}".format(power_en)]
            print("Warm initial")
        result1 = subrun(command1, timeout=120)
    ...
    if QC_TST_EN == 3:
        ...
        for testid in tms:
            ...
            user_input_1 = "{}\n{}\n{}\n{}\n{}".format(input_info['tester'], input_info['env'], input_info['toy_TPC'], input_info['comment'], FEMB_list)
            result = subrun(command, timeout=10000, user_input=user_input_1)  # rewrite with Popen later
            time.sleep(0.01)
            if result != None:
                resultstr = result.stdout
    ...
    if QC_TST_EN == 5:
        print(datetime.utcnow(), " : Start Cable Checkout.(takes < 180s)")
        print(datetime.utcnow(), " : New Test Item Starts, please wait...")
        print("\033[96m 0 : Initilization {} Temperature Checkout\033[0m".format(tmp))
        command = ["ssh", "root@192.168.121.123", "cd BNL_CE_WIB_SW_QC; python3 cable_assembly_chk.py {} save 5".format(slot_list)]
        user_input_1 = "{}\n{}\n{}\n{}\n{}".format(input_info['tester'], input_info['env'], input_info['toy_TPC'], input_info['comment'], FEMB_list)
        result = subrun(command, timeout=300, user_input=user_input_1)  # rewrite with Popen later
        time.sleep(0.01)
        if result != None:
    ...
    if QC_TST_EN == 6:
        ...
        subrun(["ssh", "root@192.168.121.123", "poweroff"], check=False)
        time.sleep(6)
