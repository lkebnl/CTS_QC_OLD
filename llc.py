...
#        self.wib.script.restype = ctypes.c_bool       
        ...
        self.wib.dat_set_dac.argtypes = [ctypes.c_uint16, ctypes.c_uint8, ctypes.c_uint8, ctypes.c_uint8]
...
#        return buf0_bytes, buf1_bytes
    ...
    def get_sensors(self, sensors=["WIB", "FEMB0", "FEMB1", "FEMB2", "FEMB3"]):
        ...
        if ("FEMB0" in sensors) or ("FEMB1" in sensors) or ("FEMB2" in sensors) or ("FEMB3" in sensors):
            ...
            for i in range(1, 9, 1):
                self.wib.read_ltc2991(bus, 0x4E, False, i)
                values = []
                for _ in range(10):
                ...
                bus2_ltc2991_4e_vs.append(average)

        if "FEMB0" in sensors:
            bus2_ltc2991_48_vs = []  # DCDC for FEMB0
            for i in range(1, 9, 1):
                values = []
                self.wib.read_ltc2991(bus, 0x48, False, i)
                for _ in range(10):
                    v = self.wib.read_ltc2991(bus, 0x48, False, i)
                    values.append(v)
                values = values[1:]
                average = sum(values) / len(values)
                # v = self.wib.read_ltc2991(bus, 0x48, False, i)
                bus2_ltc2991_48_vs.append(average)
    ...
    def datpower_poke(self, dev_addr, reg_addr, data, cd=-1, fe=-1):
        self.wib.datpower_poke(dev_addr, reg_addr, data, cd, fe)
    
    def datpower_peek(self, dev_addr, reg_addr, cd=-1, fe=-1):
        return self.wib.datpower_peek(dev_addr, reg_addr, cd, fe)
